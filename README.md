...
> 请在这台 Mac 上从零安装并配置 Abar：先只读检查系统是否为 Apple Silicon、macOS 14+，以及 Git、Node.js 20+、npm、Xcode Command Line Tools、Swift 6、Codex CLI 是否存在，并用 `codex login status` 检查登录；如果缺少可自动安装的普通依赖就完成安装，如果需要用户完成 Codex 浏览器登录、`xcode-select --install`、macOS 安全弹窗、权限确认或输入密码，请暂停并清楚告诉我该做什么，绝不要绕过安全或信任机制；环境满足后从 `https://github.com/lanewulll/Abar.git` 克隆最新 `main`（已有目录则先检查 Git 状态，不覆盖未提交修改），进入 Abar 目录执行项目真实支持的 `npm run setup`，确认 `~/Applications/Abar.app` 已生成并启动，再执行 `npm run hooks:preview` 读取现有 `${CODEX_HOME:-$HOME/.codex}/hooks.json`，确认 JSON 有效后执行 `npm run hooks:install` 让项目先备份再结构化合并 Abar 的 `UserPromptSubmit` 与 `Stop` Hook，绝不覆盖或删除其他 Hook；随后暂停并要求我在 Codex 中运行 `/hooks`、检查并人工信任 Abar Hook，完成后继续用 `curl http://127.0.0.1:3987/health`、`npm run doctor`、`npm test` 和 `npm run build` 验证，若端口占用、Reporter 路径失效、health 失败、未签名 App 被 macOS 阻止或额度接口返回 403，则保留其他可用功能并给出对应修复路径；最后输出已完成步骤、失败步骤、仍需我人工完成的步骤、Abar 下次启动命令 `open "$HOME/Applications/Abar.app"`、重新设置命令 `npm run setup` 与完整卸载预览命令 `npm run uninstall:full -- --dry-run`。
